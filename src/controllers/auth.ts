...
import { createForgotPasswordToken, deleteForgotPasswordToken, getUserIdFromForgotPasswordToken } from '../models/ForgotPasswordModel'
import { deleteAuthCookies, getAuthConnection, isNewInstallation, markInstalled, validatePasswordFormat } from '../services/auth'
import { IdTokenCookie, RefreshTokenCookie, SSOConnectionIdCookie } from '../util/cookie'
import { getEmailFromUserId, getOrgFromReq } from '../services/organizations'
import { createUser, getUserByEmail, getUserById, updatePassword, verifyPassword } from '../services/users'
...
export async function postRefresh(req: Request, res: Response) {
  ...
  try {
    ...
    const { idToken, refreshToken: newRefreshToken, expiresIn } = await auth.refresh(req, refreshToken)
...
export async function postOAuthCallback(req: Request, res: Response) {
  try {
    const { idToken, refreshToken, expiresIn } = await auth.processCallback(req, res, null)
...
async function sendLocalSuccessResponse(req: Request, res: Response, user: UserInterface) {
  const { idToken, refreshToken, expiresIn } = await auth.processCallback(req, res, user)
  if (!idToken) {
    return res.status(400).json({
      status: 400,
...
export async function postLogin(
  ...
  console.log('postLogin:: email, password', email, password)
...
export async function postFirstTimeRegister(
  ...
  if (!newInstallation) {
    throw new Error('An organization is already configured. Please refresh the page and try again.')
  }
...
export async function getResetPassword(req: Request<{ token: string }>, res: Response) {
