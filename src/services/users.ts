...
async function hash(password: string): Promise<string> {
  ...
  const derivedKey = await (scrypt(password, salt, HASH_LEN) as Promise<Buffer>)
...
export async function createUser(name: string, email: string, password?: string, verified: boolean = false) {
  let passwordHash = ''
